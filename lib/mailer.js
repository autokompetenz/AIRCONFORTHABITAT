...
async function sendPaymentProofToAdmin({ orderNumber, customerName, customerEmail, fileName, fileBuffer, fileMimetype }) {
  const adminEmail = process.env.ADMIN_NOTIFICATION_EMAIL || process.env.SMTP_USER;
  if (!adminEmail) return;

  const isImage = fileMimetype && fileMimetype.startsWith('image/');
  const isPdf = fileMimetype === 'application/pdf';
  const cid = `proof-${orderNumber}-${Date.now()}@airecoclim`;
  ...
  if (isImage) {
    proofBlock =
      '<div style="margin:12px 0;">' +
        ...
        '<img src="cid:' + cid + '" style="max-width:100%;max-height:400px;border:1px solid #E5E5E5;border-radius:6px;" alt="Preuve de virement" />' +
      '</div>';
  } else if (isPdf) {
    proofBlock =
      '<div style="margin:12px 0;padding:16px;background:#F0F4FF;border:1px solid #D6E4F0;border-radius:6px;text-align:center;">' +
        ...
        '<div style="font-size:12px;color:#8A9BAE;margin-top:4px;">Ouvrez la pièce jointe ci-dessous pour consulter le justificatif.</div>' +
      '</div>';
  } else {
    proofBlock =
      '<div style="margin:12px 0;padding:16px;background:#F0F4FF;border:1px solid #D6E4F0;border-radius:6px;text-align:center;">' +
  ...
  await sendMail({
    ...
    attachments: [{
      filename: fileName || 'preuve_virement',
      content: fileBuffer,
      contentType: fileMimetype,
      ...(isImage ? { cid, headers: { 'Content-Disposition': 'inline' } } : {}),
    }],
  });
}
