...
const FROM   = `"${process.env.FROM_NAME || 'AIR ECO CLIM'}" <${process.env.FROM_EMAIL || process.env.SMTP_USER}>`;
...
<td><span style="font-size:17px;font-weight:800;color:#0F1A2E;">AIR ECO CLIM</span><br><span style="font-size:9px;letter-spacing:0.25em;color:#2E86C1;text-transform:uppercase;">Brié-et-Angonnes · France</span></td>
<td align="right"><span style="display:inline-block;width:36px;height:36px;background:#0095D6;border-radius:6px;line-height:36px;text-align:center;font-size:16px;">❄</span></td>
</tr></table>
</td></tr>
...
<strong style="color:#0F1A2E;">AIR ECO CLIM</strong><br>288 Chemin du Cavin, 38320 Brié-et-Angonnes, France</span></td></tr>
</table>
</td></tr></table>
</body>
</html>`;
...
async function sendMail({ to, subject, html, text }) {
  ...
  return transporter.sendMail({
    ...
    headers: {
      ...
      'X-Mailer': 'AIR ECO CLIM Mailer v1',
...
function paymentRef(items) {
  if (!items || items.length === 0) return 'AIR ECO CLIM';
...
async function sendOrderConfirmation({ email, name, order, items }) {
  ...
  await sendMail({
    to: email,
    subject: `Commande ${order.orderNumber} — AIR ECO CLIM`,
    html,
    text: [
      `Commande confirmée — AIR ECO CLIM`,
      ``,
      `Bonjour ${name},`,
...
async function sendStatusNotification({ email, name, orderNumber, status }) {
  ...
  await sendMail({
    ...
    text: [
      ...
      ``,
      `-- AIR ECO CLIM`,
      `288 Chemin du Cavin, 38320 Brié-et-Angonnes, France`,
    ].join('\n'),
  });
}
...
async function sendReplyToCustomer({ email, name, subject, message }) {
  const html = wrapHtml('Message',
    '<div style="font-size:18px;font-weight:800;color:#2E86C1;margin-bottom:6px;">Message de AIR ECO CLIM</div>' +
    '<div style="margin-bottom:10px;">Bonjour ' + esc(name) + ',</div>' +
  ...
  await sendMail({
    to: email,
    subject: subject || 'AIR ECO CLIM — Message de notre équipe',
    html,
    text: [
      `Message de AIR ECO CLIM`,
      ``,
      `Bonjour ${name},`,
      ``,
      message,
...
async function sendPaymentProofToAdmin({ orderNumber, customerName, customerEmail, receiptUrl, fileName, fileMimetype }) {
  const adminEmail = process.env.ADMIN_NOTIFICATION_EMAIL || process.env.SMTP_USER;
  if (!adminEmail) return;
  ...
  if (isImage) {
    proofBlock =
      '<div style="margin:12px 0;">' +
        '<div style="font-size:10px;font-weight:700;letter-spacing:0.15em;text-transform:uppercase;color:#8A9BAE;margin:6px 0 4px;">Preuve jointe</div>' +
        '<img src="' + esc(receiptUrl) + '" style="max-width:100%;max-height:400px;border:1px solid #E5E5E5;border-radius:6px;" alt="Preuve de virement" />' +
      '</div>';
  } else if (isPdf) {
    proofBlock =
      '<div style="margin:12px 0;padding:16px;background:#F0F4FF;border:1px solid #D6E4F0;border-radius:6px;text-align:center;">' +
        '<div style="font-size:32px;margin-bottom:6px;">📄</div>' +
        '<div style="font-size:13px;color:#0F1A2E;font-weight:600;">Preuve de virement (PDF)</div>' +
        '<div style="margin-top:8px;"><a href="' + esc(receiptUrl) + '" style="display:inline-block;background:#2E86C1;color:#fff;text-decoration:none;font-size:12px;font-weight:700;padding:8px 18px;border-radius:5px;">Ouvrir le PDF</a></div>' +
      '</div>';
  } else {
    proofBlock =
      '<div style="margin:12px 0;padding:16px;background:#F0F4FF;border:1px solid #D6E4F0;border-radius:6px;text-align:center;">' +
        '<div style="font-size:13px;color:#0F1A2E;font-weight:600;">📎 Fichier joint : ' + esc(fileName) + '</div>' +
        '<div style="margin-top:8px;"><a href="' + esc(receiptUrl) + '" style="display:inline-block;background:#2E86C1;color:#fff;text-decoration:none;font-size:12px;font-weight:700;padding:8px 18px;border-radius:5px;">Ouvrir le fichier</a></div>' +
      '</div>';
  }

  const html = wrapHtml('Preuve de virement reçue',
    '<div style="font-size:18px;font-weight:800;color:#2E86C1;margin-bottom:8px;">📎 Preuve de virement reçue</div>' +
    '<div style="margin-bottom:10px;">Un client a envoyé une preuve de virement bancaire.</div>' +
    SEP() +
    L('Commande n°', orderNumber) +
    L('Client', customerName) +
    L('Email', customerEmail) +
    proofBlock +
    '<div style="text-align:center;margin:14px 0 6px;"><a href="' + esc(getPublicSiteUrl() + '/order/confirm/' + orderNumber) + '" style="display:inline-block;background:#2E86C1;color:#fff;text-decoration:none;font-size:12px;font-weight:700;padding:10px 22px;border-radius:5px;">Voir la commande</a></div>'
  );

  await sendMail({
    to: adminEmail,
    subject: `📎 Preuve de virement — Commande ${orderNumber}`,
    html,
    text: [
      ...
      `Email : ${customerEmail}`,
      ``,
      `Voir la preuve : ${receiptUrl}`,
      ``,
      `Voir la commande : ${getPublicSiteUrl()}/order/confirm/${orderNumber}`,
    ].join('\n'),
  });
}

async function sendNewsletter({ email, name, subject, body, product }) {
  let productBlock = '';
  if (product) {
    const productUrl = getPublicSiteUrl() + '/product/' + (product.slug || '');
    const priceStr = product.salePrice
      ? '<span style="font-size:20px;font-weight:900;color:#2E86C1;">' + formatEuro(product.salePrice) + '</span><span style="font-size:13px;color:#8A9BAE;text-decoration:line-through;margin-left:8px;">' + formatEuro(product.price) + '</span>'
      : '<span style="font-size:20px;font-weight:900;color:#2E86C1;">' + formatEuro(product.price) + '</span>';

    productBlock =
      '<div style="margin:18px 0;padding:20px;background:linear-gradient(135deg,rgba(46,134,193,0.06) 0%,rgba(0,180,216,0.06) 100%);border:1px solid rgba(46,134,193,0.15);border-radius:10px;">' +
        '<div style="font-size:10px;font-weight:800;letter-spacing:0.2em;text-transform:uppercase;color:#2E86C1;margin-bottom:12px;">Produit recommandé pour vous</div>' +
        (product.image
          ? '<img src="' + esc(product.image) + '" alt="' + esc(product.name) + '" style="width:100%;max-width:460px;height:auto;border-radius:8px;margin-bottom:14px;border:1px solid #E5E5E5;" />'
          : '') +
        '<div style="font-size:16px;font-weight:800;color:#0F1A2E;margin-bottom:4px;">' + esc(product.name) + '</div>' +
        '<div style="font-size:12px;color:#8A9BAE;margin-bottom:10px;">' + esc(product.brand || '') + (product.brand && product.model ? ' · ' : '') + esc(product.model || '') + '</div>' +
        '<div style="margin-bottom:14px;">' + priceStr + '</div>' +
        '<div style="text-align:center;"><a href="' + esc(productUrl) + '" style="display:inline-block;background:#2E86C1;color:#fff;text-decoration:none;font-size:12px;font-weight:700;padding:10px 24px;border-radius:5px;">Découvrir ce produit</a></div>' +
      '</div>';
  }

  const html = wrapHtml('Newsletter',
    '<div style="font-size:18px;font-weight:800;color:#2E86C1;margin-bottom:8px;">Newsletter AIR ECO CLIM</div>' +
    '<div style="margin-bottom:10px;">Bonjour ' + esc(name) + ',</div>' +
    '<div style="white-space:pre-wrap;font-size:14px;color:#0F1A2E;line-height:1.6;margin:10px 0;">' + esc(body) + '</div>' +
    productBlock +
    '<div style="text-align:center;margin:16px 0 6px;"><a href="' + esc(getPublicSiteUrl() + '/catalog') + '" style="display:inline-block;background:#0095D6;color:#fff;text-decoration:none;font-size:12px;font-weight:700;padding:10px 22px;border-radius:5px;">Voir tout le catalogue</a></div>'
  );

  await sendMail({
    to: email,
    subject: subject || 'Newsletter AIR ECO CLIM',
    html,
    text: [
      'Newsletter AIR ECO CLIM',
      '',
      'Bonjour ' + name + ',',
      '',
      body,
      '',
      product ? 'Produit recommandé : ' + product.name + ' — ' + formatEuro(product.salePrice || product.price) : '',
      product ? getPublicSiteUrl() + '/product/' + (product.slug || '') : '',
      '',
      'Voir le catalogue : ' + getPublicSiteUrl() + '/catalog',
      '',
      '-- AIR ECO CLIM',
      '288 Chemin du Cavin, 38320 Brié-et-Angonnes, France',
    ].join('\n'),
...
module.exports = { sendOrderConfirmation, sendAdminNotification, sendStatusNotification, sendReplyToCustomer, sendPaymentProofToAdmin, sendNewsletter };
