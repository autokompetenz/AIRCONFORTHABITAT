...
async function sendOrderConfirmation({ email, name, order, items }) {
  ...
  const html = wrapHtml('Commande reçue',
    ...
    '<div style="font-size:12px;color:#8A9BAE;margin:6px 0 2px;">Merci d\'utiliser le numéro de commande comme référence de paiement.</div>' +
  ...
  await sendMail({
    ...
    text: [
      ...
      `IBAN : ${IBAN}`,
      `BIC : ${BIC}`,
      `Merci d'utiliser le numéro de commande comme référence de paiement.`,
      ``,
