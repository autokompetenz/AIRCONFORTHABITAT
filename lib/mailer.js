...
async function sendReservationConfirmation({ email, name, reservation, puppy }) {
  ...
  const pay = reservation.paymentLabel || (reservation.paymentMethod === 'full' ? 'Intégral (-15%)' : 'Acompte 50% + solde livraison');
  ...
  const html = wrapHtml('Réservation confirmée',
    ...
    L('Livraison', dm) +
    (reservation.deliveryMethod === 'delivery' && reservation.deliveryAddress ? L('Adresse livraison', esc(reservation.deliveryAddress)) : '') +
    SEP() +
  ...
  await sendMail({
    ...
    text: [
      ...
      `Livraison : ${dm}${reservation.deliveryMethod === 'delivery' && reservation.deliveryAddress ? ' (' + reservation.deliveryAddress + ')' : ''}`,
